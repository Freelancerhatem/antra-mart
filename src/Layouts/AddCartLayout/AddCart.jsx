...
const AddCart = () => {
  ...
  useEffect(() => {
    AxiosLoader.get(`/productsbyid?id=${id}`).then((res) =>
  ...
  const handleAddToCart = () => {
    ...
    const myCartDuplicateChecker = myCartArray.some(
      (data) => data._id == productsData._id
    );
        ...
        <div>
          ...
          <div>
            ...
            <div className="flex gap-1 m-2 w-20 h-10">
              <div
                onClick={() => SetProductColor("Amber")}
                className={`bg-amber-400 w-6 h-6  rounded-full ${
                  productColor == "Amber" ? "border-4 border-white" : ""
                }`}
              ></div>
              <div
                onClick={() => SetProductColor("Red")}
                className={`bg-red-500 w-6 h-6 rounded-full ${
                  productColor == "Red" ? "border-4 border-white" : ""
                }`}
              ></div>
            </div>
          </div>
